import { useToast } from '@/components/molecules'
import { globalEnv, queryClient } from '@/configs'
import { CHECK_TX_STATUS, CHECK_TX_TYPE, queryKeys } from '@/constants'
import { eggApi } from '@/external/api/egg'
import { buyMysteryBoxInput } from '@/external/api/egg/buy-mystery-box'
import { EggType } from '@/external/api/egg/get-my-egg'
import {
	useBalance,
	useCheckTransaction,
	useDisconnectWallet,
	useGlobalLoading,
	useSendTransaction,
	useWaitForTransactionReceipt,
} from '@/hooks'
import { createCoin } from '@/utils'
import { useMutation } from '@tanstack/react-query'
import { useCallback, useEffect } from 'react'

const useOpenMysteryBox = ({
	onSuccess,
	onError,
}: {
	onSuccess?: (data: EggType) => void
	onError?: () => void
}) => {
	const { onDisconnect } = useDisconnectWallet()
	const result = useBalance({})
	const balance = result.data?.formatted || 0

	const {
		mutate,
		isPending: isLoadingOpenMysteryBox,
		isSuccess,
	} = useMutation({
		mutationFn: (params: buyMysteryBoxInput) => eggApi.buyMysteryBox(params),
		onError: (e: any) => {
			const error =
				e?.response?.data?.data || 'Sorry, process has some problem. Try again!'
			if (error?.response?.data?.status === 5) {
				onDisconnect()
			}
			toast({
				title: 'Open Mystery Box',
				description: JSON.stringify(error),
				variant: 'error',
			})
		},
	})

	const handleBuyMysteryBoxAfterTx = useCallback(
		(hash: string) => {
			if (hash) {
				mutate({ txHash: hash })
			}
		},
		[mutate]
	)

	const {
		data: hash,
		sendTransaction,
		isPending: isPendingSendTx,
		error: sendError,
	} = useSendTransaction({ onCallback: handleBuyMysteryBoxAfterTx })

	const {
		isLoading: isConfirming,
		isSuccess: isConfirmed,
		error: confirmError,
	} = useWaitForTransactionReceipt({
		hash,
	})

	const { data: checkTxData } = useCheckTransaction({
		txHash: isSuccess && hash ? hash : '',
		type: CHECK_TX_TYPE.BUY_OFFERING,
	})

	const { toast, dismiss } = useToast()

	const handleSend = async ({ amount }: { amount: number }) => {
		if (balance < amount) {
			toast({
				title: 'Open Mystery Box',
				description: 'Sorry, you do not have enough SUI to open',
				variant: 'error',
			})
		} else {
			sendTransaction({
				to: globalEnv.adminWalletAddress as any,
				value: [createCoin(amount)],
			})
		}
	}

	const handleSendSuccess = useCallback(() => {
		if (isConfirmed) {
			result.refetch()

			dismiss()

			checkTxData?.info && onSuccess && onSuccess(checkTxData.info)

			queryClient.invalidateQueries({
				queryKey: queryKeys.privateQuery,
				refetchType: 'active',
			})
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [isConfirmed, checkTxData])

	const handleSendError = useCallback(() => {
		if (sendError || confirmError) {
			const errorMsg =
				(sendError as any)?.details ||
				JSON.stringify(sendError) ||
				JSON.stringify(confirmError)
			toast({
				title: 'Open Mystery Box',
				description: errorMsg,
				variant: 'error',
			})
			onError && onError()
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [sendError, confirmError, toast])

	useEffect(() => {
		handleSendError()
	}, [handleSendError])

	useEffect(() => {
		if (isPendingSendTx) {
			toast({
				title: 'Open Mystery Box',
				description: 'Opening...',
				duration: 5000,
			})
		} else if (isConfirming) {
			toast({
				title: 'Open Mystery Box',
				description: 'Waiting for confirmation...',
				duration: 5000,
			})
		} else if (isConfirmed && isSuccess) {
			if (checkTxData?.status === CHECK_TX_STATUS.SUCCESS) {
				handleSendSuccess()
			}
		}
	}, [
		isPendingSendTx,
		isConfirming,
		isConfirmed,
		handleSendSuccess,
		toast,
		checkTxData,
		isSuccess,
	])

	useGlobalLoading(
		isPendingSendTx ||
			isLoadingOpenMysteryBox ||
			isConfirming ||
			checkTxData?.status === CHECK_TX_STATUS.WAITING_TO_CHECK
	)

	return {
		mutate: handleSend,
		isLoading:
			isPendingSendTx ||
			isLoadingOpenMysteryBox ||
			checkTxData?.status === CHECK_TX_STATUS.WAITING_TO_CHECK,
	}
}

export default useOpenMysteryBox
