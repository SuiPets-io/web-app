...
import { activeFusionInput } from '@/external/api/pet/active-fusion'
import {
	useBalance,
	useCheckTransaction,
	useDisconnectWallet,
...
const useBuyFusion = ({
	...
	const {
		mutate,
		isPending: isLoadingBuyFusion,
		isSuccess,
	} = useMutation({
		mutationFn: (params: activeFusionInput) => petApi.activeFusion(params),
		onError: (e: any) => {
			const error =
	...
	const handleBuyFusionAfterTx = useCallback(
		(hash: string) => {
	...
	const {
		...
		onCallback: handleBuyFusionAfterTx,
	})
	...
	const { data: checkTxData } = useCheckTransaction({
		...
		type: CHECK_TX_TYPE.ACTIVE_FUSION,
	})
