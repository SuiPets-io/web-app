...
export const useCloudStorage = (): {
		...
		(key) =>
			new Promise((resolve, reject) => {
				cloudStorage?.getItem(
					key,
					(error: any, value: string | PromiseLike<string>) => {
						if (!error) {
							resolve(value)
						} else {
							reject(error)
	...
	const setItem: SetItemFunction = useCallback(
		(key, value) =>
			new Promise((resolve, reject) => {
				cloudStorage?.setItem(key, value, (error: any, state: any) => {
					if (!error && state) {
						resolve()
					} else {
						reject(error)
		...
		(key) =>
			new Promise((resolve, reject) => {
				cloudStorage?.getItems(
					...
					(
						error: any,
						value: Record<string, string> | PromiseLike<Record<string, string>>
					) => {
						if (!error && value) {
							resolve(value)
						} else {
	...
	const removeItem: RemoveItemFunction = useCallback(
		(key) =>
			new Promise((resolve, reject) => {
				cloudStorage?.removeItem(key, (error: any, state: any) => {
					if (!error && state) {
						resolve()
					} else {
						reject(error)
	...
	const removeItems: RemoveItemsFunction = useCallback(
		(key) =>
			new Promise((resolve, reject) => {
				cloudStorage?.removeItems(key, (error: any, state: any) => {
					if (!error && state) {
						resolve()
					} else {
						reject(error)
		...
		() =>
			new Promise((resolve, reject) => {
				cloudStorage?.getKeys(
					(error: any, state: string[] | PromiseLike<string[]>) => {
						if (!error && state) {
							resolve(state)
						} else {
							reject(error)
